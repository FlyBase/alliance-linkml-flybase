...
# TODO(FTA-193): confirm these VocabularyTerm names + version semantics with Steven (schema PR #323 author).
# These are placeholder values for the required GeneChangeEventSlotAnnotationDTO fields. The DTO/mapping
# structure is complete; only these literal values need confirming before the export will validate.
GENE_CHANGE_EVENT_TYPE_PLACEHOLDER = 'rename'                # event_type_name for 'identity_source' (a rename).
GENE_NOMEN_COMMENT_EVENT_TYPE_PLACEHOLDER = 'rename'         # event_type_name for 'gene_nomenclature_comment' (semantics TBD).
GENE_CHANGE_EVENT_STATUS_PLACEHOLDER = 'current'            # event_status_name.
GENE_CHANGE_EVENT_VERSION_PLACEHOLDER = 1                   # current_version.
# TODO(FTA-193): confirm the 'identity_source' value delimiter against real chado data before relying on this.
# None => split on any run of whitespace (defensive default; the value is expected to hold exactly two symbols).
...
class GeneHandler(FeatureHandler):
    ...
    def map_gene_change_events(self):
        ...
        evidence from each prop's pubs via lookup_pub_curies().

        Note:
            The required event_type_name/event_status_name/current_version values are placeholders
            pending confirmation with Steven (schema PR #323 author) - see module-level TODOs.
        ...
        for gene in self.fb_data_entities.values():
            ...
            for note in nomen_notes:
                change_event = agr_datatypes.GeneChangeEventSlotAnnotationDTO(
                    gene_id,
                    GENE_NOMEN_COMMENT_EVENT_TYPE_PLACEHOLDER,
                    GENE_CHANGE_EVENT_STATUS_PLACEHOLDER,
                    GENE_CHANGE_EVENT_VERSION_PLACEHOLDER,
            ...
            for fb_prop in gene.props_by_type.get('identity_source', []):
                ...
                change_event = agr_datatypes.GeneChangeEventSlotAnnotationDTO(
                    gene_id,
                    GENE_CHANGE_EVENT_TYPE_PLACEHOLDER,
                    GENE_CHANGE_EVENT_STATUS_PLACEHOLDER,
                    GENE_CHANGE_EVENT_VERSION_PLACEHOLDER,
                    pub_curies,
                )
